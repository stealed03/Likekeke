    ...
    while True:
        ...
        try:
            ...
            async def handler(event):
                try:
                    if result_holder["event"].is_set():
                        return
                    text = event.raw_text or ""
                    # Catch any response related to our like command
                    keywords = ["likes sent", "daily limit", "please wait", 
                                "like request", "remain count", "next reset",
                                "likes given", "before likes", "after likes"]
