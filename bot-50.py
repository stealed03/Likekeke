...
def admin_users_kb(users, page=0, per_page=5):
    ...
    for u in chunk:
        ...
        toggle = "⏹" if u["task_active"] else "▶️"
        rows.append([
            ...
            InlineKeyboardButton(
                text=toggle,
                callback_data=f"admin_toggle_{u['user_id']}_{page}"
            )
        ])
    nav = []
    if page > 0:
...
async def cb_admin_toggle(cb: CallbackQuery):
    ...
    if user["task_active"]:
        upsert_user(uid, task_active=0)
        try: await bot.send_message(uid, "⚡ Admin ne aapka task stop kar diya!")
        except: pass
        ...
        ensure_task(uid)
        try: await bot.send_message(uid, "⚡ Admin ne aapka task start kar diya!")
        except: pass
        await cb.answer("▶️ Started!")
